...
volatile irparams_t irparams[2];//SN:create 2 irparam structures
...
IRrecv::IRrecv(int recvpin1, int recvpin2)
{
  irparams[0].recvpin = recvpin1;
  irparams[0].blinkflag = 0;
  irparams[1].recvpin = recvpin2;
  irparams[1].blinkflag = 0;
}
...
void IRrecv::enableIRIn() {
  ...
  irparams[0].rcvstate = STATE_IDLE;
  irparams[0].rawlen = 0;
  irparams[1].rcvstate = STATE_IDLE;
  irparams[1].rawlen = 0;


  // set pin modes
  pinMode(irparams[0].recvpin, INPUT);
  pinMode(irparams[1].recvpin, INPUT);
}

// enable/disable blinking of pin 13 on IR processing
void IRrecv::blink13(int blinkflag1,int blinkflag2)
{
  irparams[0].blinkflag = blinkflag1;
  irparams[1].blinkflag = blinkflag2;
...
#endif
{
  ...
  uint8_t irdata[2] = {(uint8_t)digitalRead(irparams[0].recvpin),(uint8_t)digitalRead(irparams[1].recvpin)};
  //uint8_t irdata2 = (uint8_t)digitalRead(irparams[1].recvpin);
  int i;
  irparams[0].timer++; // One more 50us tick
  irparams[1].timer++;
  for(i=0;i<=1;i++)
  ...
  case STATE_MARK: // timing MARK
    if (irdata[i] == SPACE) {   // MARK ended, record time
      irparams[i].rawbuf[irparams[i].rawlen++] = irparams[i].timer;
      irparams[i].timer = 0;
  ...
  case STATE_SPACE: // timing SPACE
    if (irdata[i] == MARK) { // SPACE just ended, record it
      irparams[i].rawbuf[irparams[i].rawlen++] = irparams[i].timer;
      irparams[i].timer = 0;
      irparams[i].rcvstate = STATE_MARK;
    } 
  ...
  case STATE_STOP: // waiting, measuring gap
    if (irdata[i] == MARK) { // reset gap timer
      irparams[i].timer = 0;
    }
    break;
  }
...
int IRrecv::decode(decode_results *results1,decode_results *results2) {
  results1->rawbuf = irparams[0].rawbuf;
  results1->rawlen = irparams[0].rawlen;
  results2->rawbuf = irparams[1].rawbuf;
  results2->rawlen = irparams[1].rawlen;
  if (irparams[0].rcvstate != STATE_STOP && irparams[1].rcvstate != STATE_STOP) {
    return ERR;
  }
#ifdef DEBUG
  Serial.println("Attempting NEC decode");
#endif
  if (decodeNEC(results1,0)||decodeNEC(results2,1)) {
    return DECODED;
  }
  /*
#ifdef DEBUG
...
#endif
  if (decodeSony(results1)) {
    return DECODED;
  }
#ifdef DEBUG
  Serial.println("Attempting Sanyo decode");
#endif
  if (decodeSanyo(results1)) {
    return DECODED;
  }
#ifdef DEBUG
  Serial.println("Attempting Mitsubishi decode");
#endif
  if (decodeMitsubishi(results1)) {
    return DECODED;
  }
#ifdef DEBUG
  Serial.println("Attempting RC5 decode");
#endif  
  if (decodeRC5(results1)) {
    return DECODED;
  }
#ifdef DEBUG
  Serial.println("Attempting RC6 decode");
#endif 
  if (decodeRC6(results1)) {
    return DECODED;
  }
#ifdef DEBUG
    Serial.println("Attempting Panasonic decode");
#endif 
    if (decodePanasonic(results1)) {
        return DECODED;
    }
#ifdef DEBUG
    Serial.println("Attempting Samsung2 decode");
#endif 
    if (decodeSamsung2(results1)) {
        return DECODED;
    }
#ifdef DEBUG
    Serial.println("Attempting JVC decode");
#endif 
    if (decodeJVC(results1)) {
        return DECODED;
    }
#ifdef DEBUG
    Serial.println("Attempting Samsung decode");
#endif 
    if (decodeSamsung(results1)) {
        return DECODED;
    }
...
long IRrecv::decodeNEC(decode_results *results, int item) {
  long data = 0;
  ...
  if (irparams[item].rawlen < 2 * NEC_BITS + 4) {
    return ERR;
  }
  // Initial space  
...
long IRrecv::decodeSony(decode_results *results, int item) {
  long data = 0;
  if (irparams[item].rawlen < 2 * SONY_BITS + 2) {
    return ERR;
  }
  ...
  while (offset + 1 < irparams[item].rawlen) {
...
long IRrecv::decodeSanyo(decode_results *results, int item) {
  long data = 0;
  if (irparams[item].rawlen < 2 * SANYO_BITS + 2) {
    return ERR;
  }
...
// Looks like Sony except for timings, 48 chars of data and time/space different
long IRrecv::decodeMitsubishi(decode_results *results, int item) {
  // Serial.print("?!? decoding Mitsubishi:");Serial.print(irparams.rawlen); Serial.print(" want "); Serial.println( 2 * MITSUBISHI_BITS + 2);
  long data = 0;
  if (irparams[item].rawlen < 2 * MITSUBISHI_BITS + 2) {
    return ERR;
  }
  int offset = 0; // Skip first space
  // Initial space  
...
long IRrecv::decodeRC5(decode_results *results, int item) {
  if (irparams[item].rawlen < MIN_RC5_SAMPLES + 2) {
    return ERR;
  }
  ...
  if (getRClevel(results, &offset, &used, RC5_T1) != MARK) return ERR;
  int nbits;
  for (nbits = 0; offset < irparams[item].rawlen; nbits++) {
...
long IRrecv::decodeRC6(decode_results *results, int item) {
...
long IRrecv::decodePanasonic(decode_results *results, int item) {
...
long IRrecv::decodeJVC(decode_results *results, int item) {
    long data = 0;
    int offset = 1; // Skip first space
    // Check for repeat
    if (irparams[item].rawlen - 1 == 33 &&
        MATCH_MARK(results->rawbuf[offset], JVC_BIT_MARK) &&
        MATCH_MARK(results->rawbuf[irparams[item].rawlen-1], JVC_BIT_MARK)) {
        results->bits = 0;
        results->value = REPEAT;
    ...
    offset++; 
    if (irparams[item].rawlen < 2 * JVC_BITS + 1 ) {
        return ERR;
    }
    // Initial space 
...
long IRrecv::decodeSamsung(decode_results *results, int item) {
 long data = 0;
 int offset = 1; // Skip first space

 // Check bits
if (irparams[item].rawlen < 2 * SAMSUNG_BITS + 4 && irparams[item].rawlen != 6) {
	return ERR;
 }
 ...
 for (int i = 0; i < SAMSUNG_BITS; i++) {
	 ...
	 offset++;	 
	 if (offset == irparams[item].rawlen)
	 {
 ...
 */
long IRrecv::decodeHash(decode_results *results, int item) {
...
long IRrecv::decodeSamsung2(decode_results *results, int item) {
    unsigned long long data = 0;
    int offset = 1;
    
	if (irparams[item].rawlen < 78)
	{
		return ERR;
	}
